...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
    vector<int> prices(keyboards.size() * drives.size(), -1);
    int most_expensive = -1;
    for(int price_drive: drives){
        for(int price_keyboard: keyboards){
            int sum_of_pices = price_drive + price_keyboard;
            prices.push_back(sum_of_pices);
        }
    }
